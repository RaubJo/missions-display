...
loading_window_layout = [[PSG.Image(filename="logo.png",size=(250,250),key="-logo-"), PSG.Text("NHBC Missions Display")],
          [PSG.Output(size=(loading_window_width-20,10),key="-output-")],
          [PSG.ProgressBar(100,size=(loading_window_width-20,10),orientation="h",key="-progress-bar-")]]


loading_window = PSG.Window(title="",no_titlebar=True, layout=loading_window_layout, size=(loading_window_width,loading_window_height), finalize=True)
...
for r in range(len(srcImgs_to_convert)):
    fname = resized_imgs+os.path.basename(srcImgs_to_convert[r]).split(".")[0]+".png"
...
layout = [[PSG.Image(filename=slides[slide],pad=(0,0), size=(width,height), key="-Image-")],]

window = PSG.Window('Test Window', layout, enable_close_attempted_event=True, no_titlebar=True, location = (0, 0),margins=(0,0), finalize=True)

window.set_cursor("no")

while True:
    event, values = window.read(timeout=display_minutes*60*1000)
    print(event, values)
    updatePSG()
